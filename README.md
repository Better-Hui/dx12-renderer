...
- The `rtas` automation scenario updates one non-stress scene mesh in place: it uploads changed vertices, refits the dirty BLAS and then updates the TLAS without changing its GPU virtual address, before disabling the path and verifying a restore frame. Meshlet GBuffer is intentionally disabled while this validation path is active because the meshlet instance-transform buffer is not yet dynamically synchronized.
