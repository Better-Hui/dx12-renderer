...
The LowPolyStreet showreel is enabled by default and starts with a six-second Meshlet cluster-debug stage. It then presents five non-looping six-second stages: noisy Path Tracing, ReSTIR DI RIS only, ReSTIR DI with temporal reuse, ReSTIR DI with temporal plus spatial reuse, and NRD with Bloom enabled. The complete sequence is 36 seconds; `F10` pauses playback and a second press restarts it from the first stage. `RAYTRACING_DEMO_SHOWREEL=0` disables the sequence for unattended runs.
