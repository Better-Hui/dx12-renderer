...
OIDN is a CPU-only static-image path. Selecting OIDN automatically enables the effective accumulation path; the ordinary `Accumulation` checkbox does not need to be enabled. Once accumulation reaches the selected static SPP, `OIDN HDR Readback` copies the preserved, converged HDR `HistoryColor` to a readback ring under the RenderGraph `COPY_SOURCE` plan. The frame thread only polls the Direct fence and never waits; a background `std::jthread` runs the OIDN CPU filter. The graph then records `OIDN Result Upload` to a persistent imported output and `OIDN Composite` to `SceneColor`. After upload, the result is held and reused on every static frame, so the noisy path no longer replaces it. Camera motion, a real scene/input reset, algorithm changes, or resource recreation advances a generation, immediately invalidates the held result, and starts a new static accumulation; stale readbacks/jobs/results are discarded. `RAYTRACING_DEMO_AUTOTEST=oidn` validates both static-result holding and camera-motion invalidation without input injection.
...
| Denoising | Optional NRD/SVGF temporal integration plus asynchronous CPU OIDN for converged static accumulation. |
...
| [Intel Open Image Denoise (OIDN)](https://github.com/OpenImageDenoise/oidn) | Git submodule at `External/OIDN/`, pinned to `v2.5.1` | CPU-only static-image denoiser. CMake builds it in an isolated build tree attached privately to `Framework`; the demo deploys the OIDN CPU/runtime DLLs without adding an OIDN solution project. |
