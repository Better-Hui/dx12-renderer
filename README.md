...
Path-tracing environment lighting currently has no direct-light NEE entry in `DirectLightCdf`: directional, point, spot, rectangular, and emissive-mesh lights are sampled there, while the environment map is evaluated when a path misses. Direct-only composite now adds a small skylight diffuse term so ground outside local-light coverage is not completely black; this is still an approximation, not full environment NEE, and is unrelated to the ground material import.
