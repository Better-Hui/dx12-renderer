...
Pass input/output declarations drive ordering, resource states, and cross-queue waits. `AddPass` creates a Direct pass; `AddComputePass` selects Async Compute when available and otherwise falls back to Direct; `AddCopyPass` requires a Copy queue; `AddExternalPass` is always Direct. The graph tracks each resource's producer queue and submitted fence value, and inserts GPU-side waits for dependent consumers. The unattended `Copy Queue Validation` path exercises `Direct HDR -> Copy -> Async Compute -> Direct` plus a real `Direct transient -> Copy transient -> Async Compute transient` heap-reuse chain. It asserts producer fences, Direct-queue alias barriers, both wait legs, state plans, submission batches, and merged heap-retirement fences. Inline-ray-query `Indirect Lighting` is the current production async-compute sample path.
...
`SceneImporter` dispatches by case-insensitive extension. Unity YAML and project JSON keep their existing asset conventions; FBX is imported as a complete scene through Assimp. A node that instances several meshes creates one `SceneObject` per submesh and stores the original Assimp mesh index in `SceneMeshReference::SubmeshIndex`, so the demo does not depend on duplicate or missing mesh names. Embedded FBX textures are decoded from memory by `TextureLoader` and participate in the same cache as file textures. Supported Mitsuba XML expands scene-local `<default>` variables, imports a perspective sensor, OBJ shapes and their `to_world` matrices, rectangle area emitters, and top-level `spot` emitters with `intensity`, `cutoffAngle`, `beamWidth`, and optional finite `range`. The right-to-left-handed reflection maps the Mitsuba sensor's local `+Z` view axis to the renderer's local `-Z` axis before the left-handed camera is constructed, preserving the source view direction. Its intentionally bounded material conversion unwraps `twosided`/`mask`/`bumpmap`, maps diffuse/plastic/conductor/dielectric families into metallic/roughness heuristics, and imports constant base colors plus bitmap base-color bindings (`reflectance`, `diffuse_reflectance`, and `base_color`). It does not reproduce complete Mitsuba transmission, alpha, bump/normal, spectral-IOR, or other advanced BSDF semantics. Relative OBJ and texture paths may not escape the XML scene directory. `Assets/Scenes/LowPolyStreet/LowPolyStreet.fbx` is the Demo default scene; its runtime sidecar preserves the Blender camera view and selects the repository's `meadow_2_4k.exr` skybox without relying on a machine-local path.

`<scene>.runtime.json` can also override material factors and maps by stable `SourceId`, falling back to a unique material name only when no stable ID is available. Supported fields are `baseColor`, `specColor`, `emissionColor`, `metallic`, `roughness`, `normalScale`, `occlusionStrength`, and every existing PBR texture binding; relative paths resolve from the sidecar directory and `Save Scene` preserves the complete override set. This repairs interchange-format loss without scene-name conditionals. In particular, Blender pure `Emission` nodes degrade to a default gray Principled material in FBX, so the LowPolyStreet sidecar restores the windows, neon, and vehicle lights as linear emissive materials. Blender's FBX exporter also stores Principled metallic in the legacy `ReflectionFactor`; the FBX importer uses that value only when Assimp exposes no native metallic factor and restores the dielectric F0 fallback to `0.04`. FBX cannot preserve an arbitrary Blender shader graph, EEVEE's lighting approximation, or AgX/Filmic display processing; camera silhouettes can therefore match exactly while final exposure and pixels are not promised to match Blender.

External FBX textures are resolved from the authored reference, the FBX directory, a sibling `textures` directory, or the parent asset's `textures` directory. If an authored extension no longer exists, the importer may use a unique file with the same stem; a unique numeric version suffix such as `_2.0` is also accepted. A source FBX submesh is uploaded once and shared by all `SceneObject` transforms/materials that instance it, so Blender Dupli-style scenes do not duplicate GPU geometry, BLAS input, or meshlet construction per object. `RAYTRACING_DEMO_SCENE` selects a scene file at startup, while `RAYTRACING_DEMO_SKYBOX` optionally replaces its skybox with an absolute path or a path relative to the scene directory.

Scene-specific runtime logic stays in the Demo. `RaytracingDemoSceneRuntimeController` selects an optional `OnLoad`/`OnUpdate`/`OnUnload` behavior from the normalized source-scene stem, while Framework `Scene` remains data-only. The LowPolyStreet behavior runs on the main thread. It derives a fixed house center from the `BUILDING*` scene objects and uses that point as both the look-at target and cone apex; the camera smoothly expands from its imported pose to a circular path on the perpendicular base plane with a 15-degree default half-angle. Three imported area lights plus three lower interleaved point lights independently orbit the house in the XZ plane. It is active only for `LowPolyStreet.fbx`; `F12` releases or restores only the scripted camera while the lights continue, and restoring control sets the scripted angular speed to the `1.0 rad/s` maximum. `Up`/`Down` adjust the scripted camera angular speed in `0.05 rad/s` steps, clamped to `0.05-1.0 rad/s`; `Left`/`Right` adjust the cone half-angle in one-degree steps over `2-30 degrees`; `W`/`S` remain manual forward/backward controls. `RAYTRACING_DEMO_SCENE_BEHAVIOR=0` disables the complete behavior for unattended static tests. An active behavior resets accumulation plus ReSTIR and OIDN static-image history because lighting inputs change every frame.

The LowPolyStreet showreel is enabled by default for this scene, but waits at its first Path Tracing stage until explicitly started. It presents five non-looping six-second stages: noisy Path Tracing, ReSTIR DI RIS only, ReSTIR DI with temporal reuse, ReSTIR DI with temporal plus spatial reuse, and the same spatial-reuse stage with NRD and Bloom enabled. Press `F10` once to reset to the first stage and play one 30-second sequence; additional presses while it is playing are ignored, and pressing `F10` after completion starts another sequence. `RAYTRACING_DEMO_SHOWREEL=0` disables the sequence for unattended runs.

Runtime rendering settings use a layered INI contract. `Config/RaytracingDemo.ini` is the complete deployed default; `Saved/RaytracingDemo.ini` is loaded afterward as the persistent user override, and environment variables retain highest priority. The ImGui `Save Settings` action writes the user file through a temporary file and atomic replacement, covering Renderer, Camera, Auto Exposure, ReSTIR DI/GI, NRD, SVGF, OIDN, Bloom, DLSS/HDR, and Diagnostics controls. Scene-owned object, light, material, skybox, and camera-pose arrays remain in `<scene>.runtime.json` through `Save Scene` instead of leaking per-scene data into the global renderer INI.

Path-tracing environment lighting currently has no direct-light NEE entry in `DirectLightCdf`: directional, point, spot, rectangular, and emissive-mesh lights are sampled there, while the environment map is evaluated when a path misses. Consequently `Bounces = 1` is intentionally local-light-only; the ground should still receive imported area/point lights, so a black ground in this mode indicates a direct-light sampling, visibility, normal, or material issue rather than missing indirect environment lighting.
...
- Transient resources are retired using the actual Direct/Async Compute/Copy fence values recorded for the frame. Two non-overlapping, single-queue lifetimes may alias across queues only when the compiler proves a RenderGraph dependency path from the former resource's last pass to the latter resource's first pass. The Direct queue waits for the producer, records the alias barrier, and signals a preamble fence that a non-Direct consumer must wait for; all queue fences touching the shared heap are merged for retirement. Multi-queue lifetimes remain deliberately non-aliasable.
...
- ReSTIR DI is an experimental inline-ray-query direct-lighting sample. Temporal candidates now compare historical/current diffuse, specular, roughness, metallic, and AO in addition to normal and depth, preventing permutation search from reusing a reservoir across coplanar but materially different pixels and turning metal or asphalt visually gray. Its light sampling, emissive surface emitters, temporal/spatial reuse, and visibility-test options continue to evolve; image quality, stability, and performance have not been accepted as an RTXDI-equivalent implementation.
