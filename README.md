...
The LowPolyStreet showreel is enabled by default for this scene. It presents five non-looping six-second stages: noisy Path Tracing, ReSTIR DI RIS only, ReSTIR DI with temporal reuse, ReSTIR DI with temporal plus spatial reuse, and the same spatial-reuse stage with NRD enabled. `F10` pauses/resumes playback; after the 30-second sequence finishes, `F10` starts it again from the first stage. `RAYTRACING_DEMO_SHOWREEL=0` disables the sequence for unattended runs.
