...
- The unattended `rtas` scenario updates one non-stress mesh's vertex buffer and transform through declared graph accesses, refits its dirty BLAS, and then updates the existing TLAS. It verifies the restore frame after disabling; Meshlet GBuffer stays disabled for this path until dynamic meshlet-instance synchronization exists.
