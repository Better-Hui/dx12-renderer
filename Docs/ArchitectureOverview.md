...
LowPolyStreet additionally uses `Left`/`Right` to adjust the `2-30 degree` cone half-angle in one-degree steps; `Up`/`Down` retain camera-speed control. Demo rendering configuration is layered and atomically persisted: executable-relative `Config/RaytracingDemo.ini` is overlaid by `Saved/RaytracingDemo.ini`, then automation environment variables are applied. `Save Settings` serializes every ImGui control, including pre-initialization NRD/SVGF settings, sky and light-group controls, scene directional/point/spot/area lights, and new-light defaults. `Save Scene` remains responsible for camera poses and scene asset data in the runtime sidecar.
