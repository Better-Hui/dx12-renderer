...
- Meshlet visibility uses a four-stage GPU pipeline: instance coarse culling, candidate-Meshlet expansion, Wave/Group-Compaction fine culling, then Compute/IA or Task/Mesh GBuffer consumption of a contiguous visible list. Intermediate stages use fixed upper-bound dispatch; only the final backend consumes GPU-generated indirect arguments.
...
- CUDA interop wraps shared D3D12 resources and external fence/semaphore synchronization. Demo-owned `BloomController` selects either the Framework raster Bloom subgraph or the CUDA external backend; only the CUDA backend is an interop consumer.
...
`SceneImporter::ImportFromFile()` is the format-neutral static entry point used by the demo. It dispatches `.unity` to the Unity YAML parser, `.json` to the project scene parser, `.fbx` to the Assimp FBX scene importer, and supported `.xml` files to the Mitsuba XML importer. All formats produce the same `Scene` contract:
...
External texture relocation covers the authored path, the FBX directory, adjacent/parent `textures` directories, unique same-stem extension changes, and unique numeric version suffixes. Scene-to-GPU conversion caches the geometry index per imported asset and source submesh: repeated scene nodes share one uploaded mesh and BLAS input while retaining independent transforms and materials. `RAYTRACING_DEMO_SCENE` and `RAYTRACING_DEMO_SKYBOX` expose these choices to unattended launches without changing source assets. `<scene>.runtime.json` can override PBR factors, emission, and texture bindings by stable `SourceId` or unique material name. This restores interchange loss such as Blender pure `Emission` nodes that FBX cannot represent, and `Save Scene` preserves the complete material override set. For Blender FBX PBR materials, the importer maps legacy `ReflectionFactor` to metallic only when Assimp has no native metallic factor, and uses dielectric F0 `0.04` instead of Blender FBX's base-color-shaped legacy specular field.

The Mitsuba XML importer is intentionally a compact compatibility path: it expands scene-local `<default>` variables, supports perspective sensors, OBJ shapes and their `to_world` matrices, rectangle area emitters, and top-level `spot` emitters (`intensity`, `cutoffAngle`, `beamWidth`, and an optional finite `range`). It converts the coordinate convention for the DirectX runtime; specifically, the reflection maps a Mitsuba sensor's local `+Z` view axis to the renderer's local `-Z` axis before constructing the left-handed camera, preserving its original world-space view direction. Its deliberately bounded PBR conversion unwraps `twosided`, `mask`, and `bumpmap`; maps diffuse/plastic/conductor/dielectric families to metallic/roughness heuristics; and imports constant base colors plus bitmap bindings for `reflectance`, `diffuse_reflectance`, and `base_color`. It does not replicate complete Mitsuba transmission, alpha, bump/normal, spectral-IOR, or other advanced BSDF semantics. Relative OBJ and texture references are constrained to the XML scene directory. `Assets/Scenes/LowPolyStreet/LowPolyStreet.fbx` is the Demo default; its runtime sidecar preserves the Blender camera view and references `Assets/Textures/skybox/meadow_2_4k.exr`, keeping the installed demo self-contained.

Per-scene lifecycle is a Demo composition concern rather than a Framework `Scene` responsibility. `RaytracingDemoSceneRuntimeController` owns an optional source-stem-selected behavior and calls its `OnLoad`, main-thread `OnUpdate`, and `OnUnload` hooks. The LowPolyStreet-only showreel derives the fixed house center from `BUILDING*` scene objects and uses it as the look-at target and cone apex. Its two-second smooth ramp reaches a 15-degree default half-angle circular path on the perpendicular base plane. The three area and three lower interleaved point lights orbit separately in the house XZ plane. It updates light buffers through `SceneLightManager` and never stores a RenderGraph builder. `F12` changes only camera ownership and sets the scripted angular speed to its `1.0 rad/s` maximum when control is restored. `Up`/`Down` adjust the scripted camera angular speed in `0.05 rad/s` steps, clamped to `0.05-1.0 rad/s`, while `W`/`S` remain manual forward/backward controls. `RAYTRACING_DEMO_SCENE_BEHAVIOR=0` disables the complete behavior for unattended static tests.

LowPolyStreet additionally uses `Left`/`Right` to adjust the `2-30 degree` cone half-angle in one-degree steps; `Up`/`Down` retain camera-speed control. Its default showreel immediately plays a six-second Meshlet cluster-debug stage followed by five six-second stages (Path Tracing noise, RIS, temporal reuse, spatial reuse, then NRD and Bloom), for 36 seconds total. `F10` pauses playback and a second press restarts from the first stage; a completed run can also be restarted with `F10`. `RAYTRACING_DEMO_SHOWREEL=0` disables it. Demo rendering configuration uses one project INI at executable-relative `Config/RaytracingDemo.ini`, with automation environment variables applied afterward. `Save Settings` atomically serializes every ImGui control back to this same file, including pre-initialization NRD/SVGF settings, sky and light-group controls, scene directional/point/spot/area lights, and new-light defaults. `Save Scene` remains responsible for camera poses and scene asset data in the runtime sidecar.
...
- Directional, point, spot, rectangular area, and emissive surface-emitter data upload through GPU buffers. Directional and point soft shadows use precompiled shader variants; spot lights apply cone falloff in direct-light sampling, while rectangular area lights sample their emitter surface.
- Optional NRD/SVGF, TAA, skybox, Framework raster Bloom, CUDA Bloom, native DLSS SR/DLAA, HDR10/PQ presentation, and experimental Streamline RR/FG paths compose around the core lighting outputs.

For ReSTIR DI, the Demo calls Framework `ReSTIRDIPass::AddPasses`. Framework registers `Initial Sampling`, `Temporal Resampling`, `Boiling Filter`, `Spatial Resampling`, and `Shade` as separate graph passes connected by tokens and imported reservoir/history resources. The Demo supplies logical scene inputs and runtime resolvers; it does not schedule the internal stages or encode their barriers. Temporal reprojection/permutation search follows RTXDI's normal and linear-view-depth compatibility test; the optional `TemporalMaterialSimilarityTest` can additionally reject diffuse, specular, roughness, metallic, and AO mismatches.
...
`BloomController` is the Demo-facing backend selector. For its raster backend, `Bloom::AddPasses` registers `Bloom Prefilter`, one pass per downsample level, one pass per upsample level, and `Bloom Composite`. The Demo supplies graph resource IDs, resolution expressions, runtime parameters, and pyramid depth; it does not allocate the pyramid or encode its barriers. Its CUDA backend remains a separate Demo-owned external queue/semaphore path.
...
HDR10 is an optional native presentation path, detected from the active `IDXGIOutput6` output. When available, the swapchain uses `R10G10B10A2_UNORM`, `DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020`, and HDR10 metadata derived from the display capabilities. The graph keeps auto-exposed scene color in `R16G16B16A16_FLOAT`; only the terminal presentation shader applies display-referred tone mapping, Rec.709-to-Rec.2020 conversion, and ST.2084/PQ encoding before writing the swapchain. This avoids an invalid format copy and keeps HDR encoding out of intermediate image-processing passes.

The request is available through `[Display] HDR10`, `RAYTRACING_DEMO_HDR10=0|1`, `--hdr10`, and the `Display Output` UI. A display without Windows HDR/PQ capability stays on the existing SDR swapchain and reports the fallback through Diagnostics (`presentation_hdr10_output`). Frame Generation and this native HDR10 path are mutually exclusive because the current Streamline-proxied presentation contract has not been validated with the HDR swapchain. HDR output expands display capability; it does not by itself correct scene-light units, exposure behavior, or a deliberately simple tone mapper.
...
- HDR10 requires both a Windows HDR-enabled HDR display and a currently compatible output. SDR remains the safe fallback. The initial HDR presentation mapping is intentionally conservative (Reinhard followed by PQ); display calibration, local adaptation, wide-gamut asset management, and HDR image-quality acceptance remain future work.
- FBX scene import is now part of the Framework `SceneImporter` contract, but it intentionally selects one active camera and supports a practical PBR subset. Spot Lights are carried through `Scene`, `LightingGpuResources`, camera counts, and Inline PT/ReSTIR DI/GI direct-light sampling; they are no longer rendered through a point-light fallback. Transparency, clearcoat, transmission, animation playback, and runtime skinning output still require separate contracts; the dynamic-scene capability reports skinned updates as unsupported until its GPU output can drive raster, Meshlet, and BLAS data together.
- `GpuReadbackBuffer` and non-blocking ring-slot `GpuReadbackTexture` are available to Framework features; compacted active-pixel validation uses them in production sample paths. `DiagnosticsImageCapture` forms the generic texture-request/image-assertion/attachment loop through an independent Direct copy, cross-frame fence `Poll()`, RGBA8 metrics, and up to two background PNG writers; terminal finalization occurs after shutdown `Drain()` so a final asynchronous image cannot be omitted. Device removal attaches `dred.txt` with its HRESULT, breadcrumbs, and page-fault allocations. On a matching NVIDIA adapter, OIDN uses D3D12 shared buffers/fences and CUDA `Quality::Fast` instead of copying HDR image pixels through the CPU; it falls back to readback/CPU `Fast` only when CUDA initialization or external-memory import fails. `DiagnosticRenderPassScope` validates SRV/UAV descriptor access against logical RenderGraph declarations; DLSS/NGX and OIDN/CUDA native D3D12 boundaries report explicit read/write observations. Cross-queue producer signals, consumer waits, state plans, and graph-resource retirement are runtime invariants that fail automation with exit code `24`. High-frequency success telemetry is sampled by semantic series, with `sampled_event_count` distinct from real capacity loss in `dropped_event_count`. Remaining work is strict RTAS/bindless/global-descriptor attribution plus whole-session background archival, compression, and retention policy.
- ReSTIR DI/GI, the CUDA Bloom backend, DLSS SR/DLAA, Streamline RR/FG, and Unity interop are engineering experiments. They require per-hardware functional, image-quality, stability, memory, and performance validation before any delivery use.
