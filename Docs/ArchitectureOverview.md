...
LowPolyStreet additionally uses `Left`/`Right` to adjust the `2-30 degree` cone half-angle in one-degree steps; `Up`/`Down` retain camera-speed control. Its default showreel is a non-looping five-stage sequence of six seconds each (Path Tracing noise, RIS, temporal reuse, spatial reuse, then NRD); `F10` pauses/resumes it and restarts it after completion, while `RAYTRACING_DEMO_SHOWREEL=0` disables it. Demo rendering configuration uses one project INI at executable-relative `Config/RaytracingDemo.ini`, with automation environment variables applied afterward. `Save Settings` atomically serializes every ImGui control back to this same file, including pre-initialization NRD/SVGF settings, sky and light-group controls, scene directional/point/spot/area lights, and new-light defaults. `Save Scene` remains responsible for camera poses and scene asset data in the runtime sidecar.
