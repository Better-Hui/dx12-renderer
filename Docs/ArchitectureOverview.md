...
LowPolyStreet additionally uses `Left`/`Right` to adjust the `2-30 degree` cone half-angle in one-degree steps; `Up`/`Down` retain camera-speed control. Its default showreel waits at the first stage until triggered, then plays one non-looping five-stage sequence of six seconds each (Path Tracing noise, RIS, temporal reuse, spatial reuse, then NRD and Bloom); `F10` resets it to the first stage and starts one 30-second run, ignores repeated presses while playing, and starts another run after completion, while `RAYTRACING_DEMO_SHOWREEL=0` disables it. Demo rendering configuration uses one project INI at executable-relative `Config/RaytracingDemo.ini`, with automation environment variables applied afterward. `Save Settings` atomically serializes every ImGui control back to this same file, including pre-initialization NRD/SVGF settings, sky and light-group controls, scene directional/point/spot/area lights, and new-light defaults. `Save Scene` remains responsible for camera poses and scene asset data in the runtime sidecar.
