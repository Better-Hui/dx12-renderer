...
- Construction-scoped feature scratch textures are declared with `RenderGraphBuilder::CreateTexture()` and released to the composition root as graph texture descriptions. Raster Bloom uses this for its downsample/upsample pyramid; the resources are graph-owned but currently dedicated because transient aliasing of the chain reproducibly hangs the device after repeated graph rebuilds.
