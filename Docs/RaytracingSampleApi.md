...
- RIS candidate count and optional initial visibility (disabled by default; enabling it can discard an occluded selected candidate before temporal/spatial reuse instead of replacing it).
...
- Boiling filtering within the temporal shader, after temporal resampling and before spatial resampling. It preserves the selected sample and caps only an outlier reservoir weight; clearing the reservoir was found to erase all direct lighting on glossy/metal surfaces and make them appear diffuse.
...
- FBX external texture relocation searches the scene and adjacent `textures` directories, then permits only unique same-stem extension or numeric-version matches. Scene GPU conversion caches each asset/source-submesh geometry and shares it across transform/material instances. Startup automation can select the file and an optional skybox override through `RAYTRACING_DEMO_SCENE` and `RAYTRACING_DEMO_SKYBOX`; a scene `.runtime.json` sidecar can persist a relocatable `skybox.texture` path and material overrides relative to the sidecar directory. Material entries resolve by stable `SourceId` first and otherwise require a unique name; factors, emission, and every existing PBR map binding are supported and preserved by `Save Scene`. Blender FBX `ReflectionFactor` is a fallback for a missing native metallic factor, and the corresponding dielectric specular fallback is F0 `0.04`.
- Framework `Scene` remains a data contract. Demo-only `RaytracingDemoSceneRuntimeController` selects optional source-stem-specific lifecycle behavior without adding scene-name branches to the importer or Framework. The current LowPolyStreet showreel derives a fixed house center from `BUILDING*` scene objects, uses it as the look-at target and the apex of a 10-degree-half-angle cone path, and orbits its six lights independently in XZ. It runs on the main thread; `F12` releases/restores only camera ownership. `Up`/`Down` adjust scripted camera angular speed from `0.05` to `1.0 rad/s` in `0.05 rad/s` steps, `Left`/`Right` adjust the `2-30 degree` cone half-angle in one-degree steps, and `W`/`S` retain manual forward/backward movement. The default showreel immediately plays a six-second Meshlet cluster-debug stage followed by five six-second stages (Path Tracing, RIS, temporal reuse, spatial reuse, NRD and Bloom), for 36 seconds total; `F10` pauses playback and a second press restarts from the first stage, including after completion. `RAYTRACING_DEMO_SHOWREEL=0` disables it. `RAYTRACING_DEMO_SCENE_BEHAVIOR=0` disables the complete behavior for unattended static accumulation.
- `Config/RaytracingDemo.ini` is the single project configuration file. `Save Settings` atomically serializes renderer, camera-control, lighting-algorithm, denoiser, post-process, upscaling/display, Diagnostics UI settings, sky/light-group controls, all editable scene lights, and new-light defaults. Camera poses and scene asset data remain in `<scene>.runtime.json` through `Save Scene`.
- ReSTIR DI temporal reuse follows RTXDI's default normal/linear-view-depth compatibility test. The previous implementation compared camera-to-point Euclidean distances, which over-rejected valid history under perspective projection; current temporal and spatial tests use the camera-forward projection of the G-buffer position. An optional material similarity test (diffuse, specular, roughness, metallic, and AO deltas) is exposed as `TemporalMaterialSimilarityTest` and is disabled by default so it does not reject valid history on material boundaries. `RAYTRACING_DEMO_RESTIRDI_TEMPORAL=0|1` and `RAYTRACING_DEMO_RESTIRDI_SPATIAL=0|1` provide no-input automation overrides for fixed A/B captures. For diagnosis only, `RAYTRACING_DEMO_RESTIRDI_TEMPORAL_IGNORE_GEOMETRY=1` keeps valid history surfaces but bypasses the normal/depth/material compatibility rejection; it is disabled by default and must not be used as a shipping quality setting.
- The environment map is not yet represented in the Path Tracing direct-light CDF. It contributes only on path miss, so `Bounces = 1` remains local-light-only. A black ground in that mode must therefore be investigated in direct-light sampling, visibility, normal, or material data; it is not an indirect-light toggle issue.
