...
- The environment map is not yet represented in the Path Tracing direct-light CDF. It contributes on path miss; direct-only composite now adds a small skylight diffuse term so surfaces outside local-light coverage are not left completely black, but this remains an approximation rather than full environment NEE.
