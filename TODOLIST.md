...
- [ ] **完整 FBX 场景导入**：当前 `Framework::ModelLoader` 已可通过 Assimp 加载 FBX mesh，sample 也在使用 FBX 资源；后续需把节点层级、局部/世界变换、PBR 材质、纹理和光源作为一个 FBX scene 契约导入 `Scene`，而不是只把 FBX 当作场景描述引用的 mesh asset。
