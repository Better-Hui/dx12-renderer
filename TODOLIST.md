...
- [ ] **多 queue 与 transient aliasing 收口**：Compiler 已能把兼容的连续 Async Compute/Copy pass 合并为同 queue non-direct batch，Diagnostics 已记录 batch/wait/signal/fence；后续补充真实 Copy queue sample 与自动化覆盖，再设计可验证的跨 queue transient aliasing。Raster Bloom 的多级 scratch 链参与现有 transient heap aliasing 时，会在反复 RG 重建后稳定触发 device hang；当前已用 graph-owned dedicated resource 保证正确性，仍需单独修复 alias activation/state/placed-heap ordering 并补压力测试。
