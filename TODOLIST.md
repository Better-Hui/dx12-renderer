...
- [ ] **Framework 诊断、自动化与 Profiler 工具**：下一优先级是在 `Framework/Diagnostics` 建立机器可读的 capture session、RenderGraph/queue/resource/descriptor snapshot、结构化 invariant、确定性 frame-step automation、CPU/GPU 关联 timing 和可复现产物。Demo 只注册 control、observation 与 scenario；测试不得注入桌面输入。设计见 `Docs/FrameworkDiagnosticsPlan.zh-CN.md`。
...
- [ ] **多 queue 资源调度**：Compiler 已能把兼容的连续 Async Compute/Copy pass 合并为同 queue non-direct batch；后续补充真实 Copy queue sample、batch/wait/signal 可观测性与自动化覆盖，再设计可验证的跨 queue transient aliasing。
