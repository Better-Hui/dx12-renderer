...
namespace
{
        ...
        DirectX::TexMetadata& metadata)
    {
        ...
        {
            throw std::invalid_argument("Memory texture data must not be empty.");
        }

        DirectX::ScratchImage image;
        if (source.IsBgra8)
        {
            const uint64_t requiredSize = static_cast<uint64_t>(source.Width) * source.Height * 4ull;
            if (source.Width == 0 || source.Height == 0 || requiredSize > source.Data.size())
            {
                throw std::invalid_argument("Raw BGRA8 memory texture has invalid dimensions or data size.");
            }
            ThrowIfFailed(image.Initialize2D(
                ...
                1u));
            const DirectX::Image* destination = image.GetImage(0u, 0u, 0u);
            const size_t sourceRowPitch = static_cast<size_t>(source.Width) * 4u;
            for (uint32_t row = 0; row < source.Height; ++row)
            {
                std::memcpy(
                    destination->pixels + static_cast<size_t>(row) * destination->rowPitch,
                    source.Data.data() + static_cast<size_t>(row) * sourceRowPitch,
                    sourceRowPitch);
            }
            metadata = image.GetMetadata();
        }
        ...
        {
            std::string format = ToLower(source.FormatHint);
            ...
            if (format == "dds")
            {
                ThrowIfFailed(DirectX::LoadFromDDSMemory(
                    source.Data.data(), source.Data.size(), DirectX::DDS_FLAGS_FORCE_RGB, &metadata, image));
            }
            else if (format == "hdr")
            {
                ThrowIfFailed(DirectX::LoadFromHDRMemory(
                    source.Data.data(), source.Data.size(), &metadata, image));
            }
            else if (format == "tga")
            {
                ThrowIfFailed(DirectX::LoadFromTGAMemory(
                    source.Data.data(), source.Data.size(), DirectX::TGA_FLAGS_NONE, &metadata, image));
            }
            else
            {
                ThrowIfFailed(DirectX::LoadFromWICMemory(
                    source.Data.data(), source.Data.size(), DirectX::WIC_FLAGS_FORCE_RGB, &metadata, image));
            }
        }
    ...
    const TextureUsageType usage)
{
    ...
    {
        throw std::invalid_argument("Memory texture cache key must not be empty.");
    }

    const D3D12TextureCacheKey cacheKey = { source.CacheKey, static_cast<uint32_t>(usage) };
    Microsoft::WRL::ComPtr<ID3D12Resource> cachedTexture;
    if (m_DeviceContext->FindCachedTexture(cacheKey, cachedTexture))
    {
        m_Uploader.AssignTextureResource(commandList, texture, cachedTexture, usage, source.CacheKey);
        return true;
    }

    DirectX::TexMetadata metadata = {};
    DirectX::ScratchImage image = DecodeTexture(source, metadata);
    if (usage == TextureUsageType::Albedo)
    {
        metadata.format = DirectX::MakeSRGB(metadata.format);
    }
    ...
    ThrowIfFailed(m_DeviceContext->GetDevice()->CreateCommittedResource(
        ...
        nullptr,
        IID_PPV_ARGS(&resource)));
    m_Uploader.AssignTextureResource(commandList, texture, resource, usage, source.CacheKey);
