...
$env:RAYTRACING_DEMO_SCENE='C:\path\to\scene.fbx'
$env:RAYTRACING_DEMO_SKYBOX='C:\path\to\skybox.exr'
...
- `Scene` from Framework stores nodes, camera, objects, materials, lights, and skybox. `SceneImporter::ImportFromFile()` is the common `.unity` / project `.json` / `.fbx` / supported Mitsuba `.xml` entry point.
...
FBX scene import uses the same Assimp geometry flags in `SceneFbxImporter` and `ModelLoader`: validation, left-handed conversion, triangulation, invalid-data filtering, four-weight limiting, and 16-bit-safe large-mesh splitting. `SceneMeshReference::SubmeshIndex` maps to `MeshPrototype::m_SourceMeshIndex`; prefer this stable index before mesh-name lookup. External and embedded FBX textures both flow through `TextureLoader`. External lookup covers the authored reference, the scene directory, `scene/textures`, and `scene.parent/textures`, with unique same-stem extension and numeric-version-suffix remapping. `RaytracingDemoSceneResources` uploads each asset/submesh geometry once and lets scene objects share it through transform/material instances; do not restore per-object `ModelLoader::Load()` calls. Spot Lights flow through `Scene`, `LightingGpuResources`, camera constants, and Inline PT/ReSTIR DI/GI direct-light sampling; they are not converted into point-light fallbacks. The importer selects one FBX camera, can generate a bounds-framing fallback camera, and does not claim advanced transparency/clearcoat/transmission or animation playback. `RAYTRACING_DEMO_SCENE` and `RAYTRACING_DEMO_SKYBOX` are the unattended startup overrides for the scene and optional skybox.

The supported Mitsuba XML path is deliberately limited: it expands scene-local `<default>` variables, imports the perspective sensor, OBJ shapes with `to_world` matrices, rectangle area emitters, and top-level `spot` emitters with `intensity`, `cutoffAngle`, `beamWidth`, and optional finite `range`. It rejects OBJ paths that escape the XML scene directory, converts the coordinate convention for DirectX, and assigns all OBJ shapes the internal `Mitsuba XML Default PBR` material. Do not add runtime interpretation of Mitsuba BSDF or texture bindings without extending the shared material contract. `Assets/Scenes/CountryKitchen/scene.xml` is the default Demo scene and is copied as a complete relative-path package after build; its 295 standalone OBJ loads are intentionally a heavy startup path pending streaming or a cooked geometry cache.
...
- Deepen the implemented Framework diagnostics contract with generic `DiagnosticsImageCapture` texture requests, non-blocking ring-slot readback, cross-frame fence polling, `image.<name>` assertions, manifest attachments, background PNG encoding, and DRED failure attachments already in place. Next, close actual shader-access versus RenderGraph-declaration validation, broaden cross-queue/lifetime invariants, and add whole-session background archival, compression, and retention policy. The machine-readable session, RenderGraph/queue/resource/descriptor telemetry, named control/observation runner, structured assertions, CPU/GPU timing, query/diff/reproduction CLI, compacted active-pixel readback, and OIDN HDR readback are already implemented; do not list that baseline as future work.
...
`RAYTRACING_DEMO_AUTOTEST=visual` is the compacted-dispatch image regression. It exercises PT Direct, PT Indirect, ReSTIR DI, ReSTIR GI, and ReSTIR DI+GI without synthetic input, verifies that active count and dispatch arguments agree, then uses `DiagnosticsImageCapture` for an asynchronous presentation-texture readback. It rejects black/near-black images through five `image.<name>` assertions and writes self-contained PNG attachments under `build\bin\Release\Saved\Diagnostics\<capture>\images`. Run it with `RAYTRACING_DEMO_RAY_TRACING_DISPATCH=compacted`; use `RendererDiagnostics run ... --scenario visual --max-events 524288` when a complete agent-readable capture is required after active-pixel compaction, descriptor-binding, indirect-dispatch, or ReSTIR compacted-path changes.
...
- The LowPolyStreet-only `LowPolyStreet.ShowreelOrbit` behavior runs from the Demo main-thread update. `F12` releases/restores scripted camera ownership while lights continue; restoring control sets the scripted angular speed to its `1.0 rad/s` maximum. `Up`/`Down` adjust camera angular speed in `0.05 rad/s` steps, clamped to `0.05-1.0 rad/s`; `Left`/`Right` adjust the cone half-angle, whose default is `15°` and clamp is `2°-30°`; `W`/`S` remain manual forward/backward controls. Keep speed integration phase-continuous so changing speed never jumps the camera orbit.
