...
- `External/OIDN` is the pinned [Open Image Denoise](https://github.com/OpenImageDenoise/oidn) submodule at `v2.5.1`. The accepted CPU-only static-image path builds it in `build/ThirdParty/OIDN` through `CMakeIncludes/BuildOidn.cmake`, privately attached to `Framework` as a pre-build command so no OIDN solution project appears. It requires oneTBB from `vcpkg install tbb:x64-windows` and the provisioned ISPC executable; deploy `OpenImageDenoise.dll`, `OpenImageDenoise_core.dll`, `OpenImageDenoise_device_cpu.dll`, and `tbb12.dll` with the demo.
...
- OIDN is a CPU-only static-image subgraph: selecting it automatically enables effective accumulation, `OIDN HDR Readback` reads the preserved HDR `HistoryColor` under the graph's `COPY_SOURCE` plan, a background `std::jthread` runs the OIDN CPU filter after a non-blocking Direct-fence poll, then `OIDN Result Upload` writes its imported persistent output and `OIDN Composite` writes `SceneColor`. After upload, the result is held and reused while the camera and scene input remain static. Camera motion, a real render-input reset, selection changes, or resource recreation must advance the OIDN generation and immediately invalidate the held result; stale readbacks/jobs/results are discarded. Do not read and later write the same logical SceneColor resource in the subgraph: this RenderGraph has no SSA versions and will correctly see that as a cycle once later passes also write SceneColor.
...
`RAYTRACING_DEMO_AUTOTEST=oidn` selects OIDN, sets static SPP to `2` while deliberately leaving manual accumulation disabled, and verifies automatic accumulation, asynchronous HDR readback, CPU filtering, result upload, static-result holding, immediate generation invalidation after a programmed camera move, and a second successful convergence before returning to NRD. It must complete with `oidn_async_pipeline=pass`, `oidn_motion_reset_immediate=pass`, and `oidn_motion_invalidation=pass` in the Diagnostics capture and without a newly written `DemoException.log` or `WindowCallbackException.log`.
